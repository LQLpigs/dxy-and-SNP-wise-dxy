    ...
    if(lastchr=="")
    {
     ...
     if( (thischr!=lastchr) || (lastwindow!=thiswindow) )
     {
         if(comparisons>0&&snps>0)
            cout << lastchr << "\t" << lastwindow*windowsize << "\t" << pop1size << "\t" << pop2size << "\t" << snps << "\t" << comparisons << "\t" << diffs << "\t" << hethetdiffs << "\t" << hethomodiffs << "\t" << homohomodiffs << "\t" << double(diffs)/double(windowsize)/(double(comparisons)/double(snps)) << "\t" << double(diffs)/double(windowsize)/double(pop1size*pop2size*4) << "\t" << double(diffs)/double(comparisons) << "\t" << double(diffs)/double(snps)/double(pop1size*pop2size*4) << endl;
         else
